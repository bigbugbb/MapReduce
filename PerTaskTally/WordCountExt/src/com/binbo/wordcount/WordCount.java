...
public class WordCount {
  ...
  public static class TokenizerMapper 
    ...
    private HashMap<Text, IntWritable> mWordMap;
      
    protected void setup(Context context) throws IOException, InterruptedException {
      mWordMap = new HashMap<Text, IntWritable>();
    }
	  ...
	  while (itr.hasMoreTokens()) {
		...
		if (c == 'm' || c == 'M' || c == 'n' || c == 'N' || c == 'o' || c == 'O' || 
          ...
          if (mWordMap.containsKey(word)) {
      	    IntWritable count = mWordMap.get(word); 
      	    count.set(count.get() + 1);
      	    mWordMap.put(word, count);        		
          } else {
        	mWordMap.put(word, new IntWritable(1));
          }
		}
      }
    }
    ...
    protected void cleanup(Context context) throws IOException, InterruptedException {
      for (Map.Entry<Text, IntWritable> entry : mWordMap.entrySet()) {     
